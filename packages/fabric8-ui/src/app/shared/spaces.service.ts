...
export class SpacesService extends RecentUtils<Space> implements Spaces {
  ...
  private initCurrent(): void {
    this._current = merge(
      this.contexts.current.pipe(map((val) => val.space)),
  ...
  private initRecent(): void {
    ...
    this.subscriptions.push(
      this.loadRecentSpaces().subscribe((spaces: Space[]): void => this._recent.next(spaces)),
    );

    this.subscriptions.push(
      this.broadcaster
        .on<Space>('spaceChanged')
        .pipe(
          withLatestFrom(this.recent),
          map(
            ([changedSpace, recentSpaces]: [Space, Space[]]): RecentData<Space> =>
              this.onBroadcastChanged(changedSpace, recentSpaces),
          ),
          filter((recentData: RecentData<Space>): boolean => recentData.isSaveRequired),
        )
        .subscribe(
          (recentData: RecentData<Space>): void => this.saveRecentSpaces(recentData.recent),
        ),
    );
    ...
    this.subscriptions.push(
      this.broadcaster
        ...
        .pipe(
          withLatestFrom(this.recent),
          map(
            ([deletedSpace, recentSpaces]): RecentData<Space> =>
  ...
  private loadRecentSpaces(): Observable<Space[]> {
    return this.profileService.current.pipe(
      switchMap(
        (profile: ExtProfile): Observable<Space[]> => {
