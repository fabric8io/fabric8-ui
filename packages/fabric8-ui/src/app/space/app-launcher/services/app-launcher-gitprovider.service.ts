...
export class AppLauncherGitproviderService implements GitProviderService {
  ...
  getDetectedBuildRuntime(repoUrl: string): Observable<BuildTool> {
    ...
    let url = this.detectorApiUrl + '/api/detect/build/' + repoUrl;
    if (this.detectorApiUrl[this.detectorApiUrl.length - 1] === '/') {
      url = this.detectorApiUrl + 'api/detect/build/' + repoUrl;
    }
