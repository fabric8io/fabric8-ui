...
export class CodebasesItemDetailsComponent implements OnDestroy, OnInit {
  ...
  ngOnInit(): void {
    if (
      this.codebase == undefined ||
      !this.codebase.gitHubRepo ||
      this.codebase.attributes.type !== 'git'
    ) {
      return;
    }
    this.subscriptions.push(
      this.gitHubService.getRepoDetailsByUrl(this.codebase.attributes.url).subscribe(
        (gitHubRepoDetails) => {
          this.codebase.gitHubRepo = {};
          ...
          this.subscriptions.push(
            this.gitHubService.getRepoLastCommitByUrl(this.codebase.attributes.url).subscribe(
          ...
          this.subscriptions.push(
            this.gitHubService.getRepoLicenseByUrl(this.codebase.attributes.url).subscribe(
              (gitHubRepoLicense) => {
  ...
  private isGitHubHtmlUrlInvalid(): boolean {
    return (
      this.codebase.attributes.url === undefined ||
