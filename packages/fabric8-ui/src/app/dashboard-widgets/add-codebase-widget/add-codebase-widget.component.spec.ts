...
@Component({
  template:
    '<fabric8-add-codebase-widget [userOwnsSpace]="userOwnsSpace"></fabric8-add-codebase-widget>',
})
class HostComponent {
...
describe('AddCodebaseWidgetComponent', () => {
  ...
  it('should enable buttons if the user owns the space', function() {
    ...
    expect(
      testContext.fixture.debugElement.query(By.css('#test-add-codebase-circle-button')),
    ).not.toBeNull();
    expect(
      testContext.fixture.debugElement.query(By.css('#test-add-codebase-button')),
    ).not.toBeNull();
  });
