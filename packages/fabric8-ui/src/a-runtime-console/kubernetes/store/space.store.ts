...
class SpaceConfigWatcher {
  ...
  constructor(
    protected configMapStore: ConfigMapStore,
    public watcher: Watcher<ConfigMaps>,
    protected onChangeFn: (SpaceConfig) => void,
  ) {
    ...
    if (resourceOperation) {
      if (resourceOperation.operation == Operation.DELETED) {
        ...
        if (configMap) {
          ...
          this.spaceConfig = new SpaceConfig(
            configMap.namespace,
...
export class SpaceStore {
  ...
  constructor(
    ...
    this.list = namespacesList.pipe(
      combineLatest(this.spaceConfigsSubject.asObservable(), this.combineNamespacesAndConfigMaps),
    );
  ...
  protected combineNamespacesAndConfigMaps(
