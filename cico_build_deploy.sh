...
mkdir -p dist && docker run --detach=true --name=fabric8-ui-builder -t -v $(pwd)/dist:/dist:Z -e BUILD_NUMBER -e BUILD_URL -e BUILD_TIMESTAMP -e JENKINS_URL -e GIT_BRANCH -e "CI=true" -e GH_TOKEN -e NPM_TOKEN -e FABRIC8_BRANDING=openshiftio -e REALM=fabric8 fabric8-ui-builder
...
docker exec fabric8-ui-builder ./run_unit_tests.sh
  ...
  if [ $? -eq 0 ]; then
    ...
    docker exec fabric8-ui-builder ./upload_to_codecov.sh
else
...
if [ $? -eq 0 ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    docker tag fabric8-ui-deploy registry.devshift.net/fabric8io/fabric8-ui:$TAG && \
    docker push registry.devshift.net/fabric8io/fabric8-ui:$TAG && \
